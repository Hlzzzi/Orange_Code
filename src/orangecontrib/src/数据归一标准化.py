...
class Widget(OWWidget):
    ...
    TZ_col_alias = ['gr', 'sp', 'lld', 'msfl', 'lls', 'ac', 'den', 'cnl', 'woh', 'rop', 'torque', 'rpm', 'wob',
                    'ssp']  # 这些列名(大写)将自动识别为特征
    ...
    def output_file_name(self) -> str:
        ...
        return datetime.now().strftime("%y%m%d%H%M%S") + '_数据归一标准化.xlsx'  # 默认保存文件名
    ...
    def ignore_function(self, text, prop):
        ...
        if text == '忽略':
            ...
            if row is not None:
                Normaltypes = ['夹板法', '切线法', '绝对值法', '去均值法', '去中值法', 'Zscore均值法', 'Zscore中值法',
                               '对数转换',
                               '反余切函数转换', '小数定标规范化']
                comboBox4 = QComboBox()
                comboBox4.addItems(Normaltypes)
                comboBox4.setCurrentText('夹板法')
                ...
                # 创建文本输入框
                line_edit = QLineEdit()
                line_edit.setPlaceholderText("默认 0,100")

                line_edit.textChanged.connect(lambda text, prop=prop: self.on_text_changed(text, prop))
                # line_edit.textChanged.connect(self.on_text_changed)
                self.leftTopTable.setCellWidget(row, 4, line_edit)

    def logssd(self, text, prop):
        if text == '指数数值':
            self.loglists.append(prop)
            print("指数数值被选择，执行相应的函数", self.loglists)
    Amplitude = 1000
    N = 8
    Small_a = 0.8
    fs = 15
    ...
    def run(self):
        ...
        # 保存到文件夹中的 Excel 文件
        excel_file_path = os.path.join(folder_path, '数据归一标准化配置文件.xlsx')
    ...
    def onComboBoxIndexChanged99(self, index):
        # 获取当前选择的文本
        selected_text = self.comboBoxleft99.currentText()
        if selected_text == 'True':
            self.normal = True
        else:
            self.normal = False
        print(f"当前选择normal是：{selected_text}", self.normal)
    ...
    def fillPropTable(self, data: pd.DataFrame, tableName: str, table: QTableWidget, typeList: list,
        ...
        for i, prop in enumerate(properties):
            ...
            comboBox.currentTextChanged.connect(lambda text, prop=prop: self.logssd(text, prop))
            ...
            if prop.lower() in self.wellname_col_alias:  # 设置井名索引
                ...
                self.comboBox4.addItems(Normaltypes)
                self.comboBox4.setCurrentText('夹板法')
                ...
                line_edit.textChanged.connect(lambda text, prop=prop: self.on_text_changed(text, prop))
                # line_edit.textChanged.connect(self.on_text_changed)
                table.setCellWidget(i, 4, line_edit)
    ...
    def on_text_changed(self, text, prop):
        sender = self.sender()
        row = self.leftTopTable.indexAt(sender.pos()).row()
        col = self.leftTopTable.indexAt(sender.pos()).column()
        ...
        if self.validate_input(text):
            ...
            print(f"Row {row}, Column {col}: {values}")
            valuselist = self.dictnames[prop]
            valuselist[1] = values
        else:
            print(f"Row {row}, Column {col}: Invalid input")
    ...
    def __init__(self):
        ...
        self.leftTopTable.horizontalHeader().setSectionResizeMode(QHeaderView.Stretch)
        self.leftTopTable.verticalHeader().hide()
        self.leftTopTable.setColumnCount(5)
        self.leftTopTable.setHorizontalHeaderLabels(['属性名', '数值类型', '作用类型', '方法', '范围'])
        ...
        self.radioGroup.buttonClicked.connect(self.on_radio_button_clicked)

        self.layoutBOTTOMrr = QVBoxLayout()
        # 创建标签
        label7 = QLabel('normal:')
        label8 = QLabel(':')
        label9 = QLabel('a:')
        label10 = QLabel('zscoreVaule:')
        label11 = QLabel('fs:')
        label12 = QLabel('max_clip:')
        label13 = QLabel('order:')

        # 创建下拉框
        self.comboBoxleft99 = QComboBox()
        self.comboBoxleft99.addItems(['True', 'False'])
        self.comboBoxleft99.currentIndexChanged.connect(self.onComboBoxIndexChanged99)
        ...
        self.input10.setPlaceholderText('请输入,默认为3')
        ...
        hbox7.addWidget(self.comboBoxleft99)
        ...
        fiii = ['Zscore均值法', 'Zscore中值法', '对数转换', '反余切函数转换', '小数定标规范化']

        self.checkboxes = []
        for filter_1name in fiii:
            checkbox9 = QRadioButton(filter_1name)
            # checkbox9.toggled.connect(self.on_radio_button_toggled)
            self.checkboxes.append(checkbox9)
            # self.layoutBOTTOMrr.addWidget(checkbox9)
            self.radioGroup.addButton(checkbox9)
        ...
        # 创建标签
        label1 = QLabel('nanlist:')
        label2 = QLabel('决策模式:')
        label3 = QLabel('N:')
        label4 = QLabel('window:')
        label5 = QLabel('Wn:')
        label6 = QLabel('Amplitude:')
        ...
        self.input1.setPlaceholderText('-9999, -999.25, -999, 999, 999.25, 9999')
        # 创建输入框
        ...
        fillters = ['夹板法', '切线法', '绝对值法', '去均值法', '去中值法']
        # self.checkboxes = []
        for filter_name in fillters:
            checkbox = QRadioButton(filter_name)
            # checkbox.toggled.connect(self.on_radio_button_toggled)
            self.checkboxes.append(checkbox)
            # self.layoutBOTTOM.addWidget(checkbox)
            self.radioGroup.addButton(checkbox)
    ...
    def on_checkbox_changed(self, state):
        self.selected_filters = [checkbox.text() for checkbox in self.checkboxes if checkbox.isChecked()]
        print("selected_filters:", self.selected_filters)

    Normaltype = None
    nanlist = [-9999, -999.25, -999, 999, 999.25, 9999]

    def on_radio_button_clicked(self, button):
        self.Normaltype = button.text()
        print("Normaltype:", self.Normaltype)
    ...
    def onTextChanged(self, text):
        ...
        if sender == self.input1:
            ...
            try:
                ...
                self.nanlist = numbers
                print("分类列表:", self.nanlist)
            except ValueError:
            ...
            text = int(text)
            self.zscoreVaule = text
            print("zscoreVaule:", self.zscoreVaule)
            print(type(self.zscoreVaule))
