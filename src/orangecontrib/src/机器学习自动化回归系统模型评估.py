...
class Widget(OWWidget):
    ...
    score_types: dict = {
        ...
        "D2 Tweedie偏差分数": "d2_tweedie_score",
        "平均弹球损失": "mean_pinball_loss",
        "Regressionscore": "Regressionscore",
        "RegressionZscore": "RegressionZscore",
        "回归误差分散率": "Regressor_Error_dispersion",
        "回归预测精度评分": "Regression_prediction_accuracy_score",
    }  # 评价指标

    loglists = ["KSDR", "KTIM", "KSDR_FFV", "KTIM_FFV", "perm", "permeability", "Perm", "PERM"]  # 指数参数
    depth_index = "depth"
    ...
    def evaluation(self, data: pd.DataFrame, model, modelname, features: list, target: str, scoretype: str):
        ...
        self.targetPredictOutputBaseFile[target] = data_log2
        if len(data_log2) <= 3:
            pass
        else:
            pred_names = []
            ...
            if y_name in loglists:
                ...
                scoring = self.get_Regressor_score(
                    data_log2[y_name], data_log2[modelname], scoretype=scoretype
                )
    ...
    def Regressor_Error_dispersion(self, y_true, y_predict):
        error_rpd = pd.DataFrame([])
        error_rpd["true"] = y_true
        error_rpd["prediction"] = y_predict
        if max(y_true) - min(y_true) == 0:
            return 0
        else:
            error_rpd["error_r"] = abs(y_predict - y_true) / (max(y_true) - min(y_true)) * 100
            score0 = np.average(abs(error_rpd["error_r"]))
            return score0

    def Regression_prediction_accuracy_score(self, y_true, y_predict):
        error_rpd = pd.DataFrame([])
        error_rpd["true"] = y_true
        error_rpd["prediction"] = y_predict
        if max(y_true) - min(y_true) == 0:
    ...
    def get_Regressor_score(self, y_true, y_pred, scoretype="mean_absolute_error"):
        ...
        elif scoretype == "Regressionscore":
            # sklearn.metrics.mean_pinball_loss(y_true, y_pred, *, sample_weight=None, alpha=0.5, multioutput='uniform_average')
            scoring = self.Regressor_score(y_true, y_pred)
        elif scoretype == "RegressionZscore":
            # sklearn.metrics.mean_pinball_loss(y_true, y_pred, *, sample_weight=None, alpha=0.5, multioutput='uniform_average')
            scoring = self.Regressor_Zscore(y_true, y_pred, n=3)
        elif scoretype == "Regressor_Error_dispersion" or scoretype == "回归误差分散率":
            scoring = self.Regressor_Error_dispersion(y_true, y_pred)
        elif scoretype == "Regression_prediction_accuracy_score" or scoretype == "回归预测精度评分":
            scoring = self.Regression_prediction_accuracy_score(y_true, y_pred)
        return scoring
