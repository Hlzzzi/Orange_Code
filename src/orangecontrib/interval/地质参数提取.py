...
class Widget(OWWidget):
    ...
    def output_file_name(self) -> str:
        ...
        return datetime.now().strftime("%y%m%d%H%M%S") + '_地质参数.xlsx'  # 默认保存文件名
    ...
    def __init__(self):
        ...
        right_label = QLabel('特征参数:')
        self.layoutright.addWidget(right_label)

        # 创建下拉框
        self.comboBoxRight = QComboBox()
        self.comboBoxRight.addItems(
            ['个数', '平均值', '标准差', '方差', '偏度', '峰度', '求和', '众数', '中位数', '上四分位数', '下四分位数',
             '最大值', '最小值', '极差', '四分位差', '离散系数'])
        self.comboBoxRight.currentTextChanged.connect(self.gettext)
        ...
        self.layoutright.addWidget(self.labelSettingBtn)
        ...
        self.input1.setPlaceholderText('100')
    ...
    def open_new_window(self):
        ...
        lllb = QLabel('选择标签（唯一）,默认是None')
        layout.addWidget(lllb)
    ...
    def checkbox_state_changed(self, state):
        ...
        if isinstance(sender, QCheckBox):
            if state == 2:  # Checked state
                ...
                self.lognames.append(sender.text())
                print(self.lognames)
            elif state == 0:  # Unchecked state
                # print("取消选中:", sender.text())
                self.lognames.remove(sender.text())
                print(self.lognames)
    ...
    def CMB1(self):
        ...
        aa = self.data[self.target].astype(str)
        self.combo_box1.addItems(aa.tolist())
        self.combo_box1.currentIndexChanged.connect(self.combo_box1_currentIndexChanged)

    labelss = None

    def combo_box1_currentIndexChanged(self, index):
        print("选择的内容:", self.combo_box1.currentText())
        self.labelss = self.combo_box1.currentText()
