...
class OWPredictions(OWWidget):
    ...
    PROB_OPTS = ["(无)",
                 "数据中的分类", "模型中的分类", "模型和数据均有的分类"]
    PROB_TOOLTIPS = ["不显示概率",
                     "显示数据中的分类",
                     "显示模型中的分类，包括本数据中没有出现的分类",
                     "显示数据中有，模型也有的分类"
                     ]
    ...
    def __init__(self):
        ...
        self.tab_store_select = gui.tabWidget(self.mainArea)
        # gui.createTabPage(self.tab_store_select,"default")
        self.tab_store_select.setFixedHeight(20)
        # 为QTabWidget设置事件
        self.tab_store_select.currentChanged.connect(self.tab_store_select_changed)
        self.mainArea.layout().addWidget(self.tab_store_select)
    ...
    def tab_store_select_changed(self,page_index:int):
        print(page_index)
        if page_index == -1:
            return
        name = self.tab_store_select.tabText(page_index)
        print(name)
        self.now_select_tab_name = name
        self.run_inputs_method(self.inputs_ids[self.tab_name[name]])
    ...
    def find_tab_index(self,target_tab_name:str):
        for i in range(self.tab_num_index):
            if target_tab_name == self.tab_store_select.tabText(i):
                return i
        return -1
    ...
    def set_data(self, data,id):
        if id in self.inputs_ids:
            # 这是断开的情况
            if data is None:
                need_to_reflash = False
                del self.inputs_ids[id]
                old_filename = self.inputs_filename[id]
                del self.tab_name[old_filename]
                tab_index = self.find_tab_index(old_filename)
                if (tab_index != -1):
                    if self.now_select_tab_name == old_filename:
                        need_to_reflash = True
                    self.tab_store_select.removeTab(tab_index)
                else:
                    self.error("页签删除失败")
                del self.inputs_filename[id]
                self.tab_num_index -= 1
                if self.tab_num_index != 0 and need_to_reflash:
                    print("删除的是当前页签")
                    self.run_inputs_method(self.inputs_ids[self.tab_name[self.tab_store_select.tabText(0)]])
                    self.tab_store_select.setCurrentIndex(0)
                    self.handleNewSignals()

            # 这是重复信号
            else:
                need_to_reflash = False
                for temp_tab_name in self.tab_name.keys():
                    temp_id = self.tab_name[temp_tab_name]
                    if temp_id == id:
                        del self.tab_name[temp_tab_name]
                        tab_index = self.find_tab_index(temp_tab_name)
                        print("pre:",temp_tab_name)
                        print("now:",self.now_select_tab_name)
                        if temp_tab_name == self.now_select_tab_name:
                            need_to_reflash = True
                        if (tab_index != -1):
                            self.tab_store_select.removeTab(tab_index)
                        else:
                            self.error("页签重置失败")
                        break
                filename = getattr(data,"name","Data") 
                while(self.tab_name.get(filename)):
                    filename += "_1"
                self.inputs_ids[id] = data
                self.inputs_filename[id] = filename
                self.tab_name[filename] = id
                gui.createTabPage(self.tab_store_select, self.inputs_filename[id])

                if need_to_reflash:
                    print("need_to_reflash")
                    new_tab_index = self.find_tab_index(filename)
                    if new_tab_index == -1:
                        self.error("更新页签失败")
                        return
                    self.run_inputs_method(self.inputs_ids[self.tab_name[self.tab_store_select.tabText(new_tab_index)]])
                    self.tab_store_select.setCurrentIndex(new_tab_index)
                    self.handleNewSignals()
        else:
            # 这是新连接
            if data is not None:
    ...
    def run_inputs_method(self,data):
    ...
    def _call_predictors(self):
        ...
        for index, slot in enumerate(self.predictors):
            ...
            if target is not self.class_var and target.is_discrete:
                ...
                pred = predictor.backmap_value(
                    pred, prob, n_values, backmappers)
    ...
    def _commit_predictions(self):
        ...
        names = [var.name for var in chain(
            attrs, self.data.domain.class_vars, metas) if var]
        uniq_newmetas = []
            ...
            for i in range(data_model.rowCount()):
                yield [data_model.headerData(i, Qt.Vertical, Qt.DisplayRole)] + \
                      [delegate.displayText(
                          predictions_model.data(
                              predictions_model.index(i, j)),
                          QLocale())
