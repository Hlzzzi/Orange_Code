...
def Filter_choice(sig, window=13, N=8, Wn=0.8, Amplitude=1000, a=0.8, A=1000, fs=15, max_clip=10, order=201,
    ...
    # sig_Filters(sig,modetype='lowpass',N=8,Wn=0.8)
        ...
        sigr = detrend_Filter(sig, N=N, Wn=Wn)
    elif modetype == '低通滤波':
...
['希尔伯特变换', '去峰滤波', '去趋势滤波', '低通滤波', '高通滤波', '带通滤波', '带阻滤波', '限幅滤波', '中位值滤波', '算术平均滤波', '递推平均滤波', '中位值平均滤波', '限幅平均滤波', '一阶滞后滤波', '加权递推平均滤波', '消抖滤波', '限幅消抖滤波法']
...
def Intelligent_filtering(logspath, lognames, window=13, N=8, Wn=0.8, Amplitude=1000, a=0.8, A=1000, fs=15, max_clip=10,
                          order=201, modetypes=['算术平均滤波法', '中位值滤波法', '加权递推平均滤波法'],
                          depth_index='depth', replace_depth_names=['DEPT', 'DEPTH', 'depth', 'Depth', '#Depth']):
    import lasio
    import os
    ...
    for i, path_name in enumerate(L):
        ...
        for logname in lognames:
            ...
            for modetype in modetypes:
                aa = Filter_choice(sig, window=window, N=N, Wn=Wn, Amplitude=Amplitude, a=a, A=A, fs=fs, order=order,
                                   modetype=modetype)
                sig = aa
            data_p[logname + '_filter'] = sig
        # print("&&&&&&&&&&&&&&&&&&&&&&&")
        # print(data_p)
...
# # ,'算术平均滤波','中位值滤波','加权递推平均滤波'
ALLDATA = Intelligent_filtering(logspath, lognames=['GR', 'SP', 'LLD', 'MSFL', 'LLS', 'AC', 'DEN', 'CNL'], N=8, window=13, Wn=0.8,
