...
class Widget(OWWidget):
    ...
    def output_file_name(self) -> str:
        ...
        return datetime.now().strftime("%y%m%d%H%M%S") + '_智能滤波器.xlsx'  # 默认保存文件名
    ...
    def __init__(self):
        ...
        self.leftTopTable.horizontalHeader().setSectionResizeMode(QHeaderView.Stretch)
        self.leftTopTable.verticalHeader().hide()
        self.leftTopTable.setColumnCount(4)
        self.leftTopTable.setHorizontalHeaderLabels(['属性名', '数值类型', '作用类型', '方法选择'])
        ...
        fiii = ['算术平均滤波', '递推平均滤波', '中位值平均滤波', '限幅平均滤波', '一阶滞后滤波',
        ...
        fillters = ['希尔伯特变换', '去峰滤波', '去趋势滤波', '低通滤波', '高通滤波', '带通滤波', '带阻滤波',
                    '限幅滤波',
                    '中位值滤波']
        # self.checkboxes = []
