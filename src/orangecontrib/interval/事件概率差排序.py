...
class Widget(OWWidget):
    ...
    def set_datalist(self, datalist):
        if datalist:
            ...
            # 创建一个文件夹来保存 Excel 文件
            folder_path = './config_Cengduan/事件概率差排序'
            os.makedirs(folder_path, exist_ok=True)  # 如果文件夹不存在，则创建它

            # 保存到文件夹中的 Excel 文件
            self.user_input = os.path.join(folder_path, '事件概率差排序配置文件.xlsx')
            print('保存配置文件到:', self.user_input)
            self.data.to_excel(self.user_input, index=False)
            self.read()
        else:
            self.data = None
    ...
    def __init__(self):
        ...
        # 创建两个多选框['事件概率分析图','事件概率差排序图']
        self.checkBox1 = QCheckBox('事件概率分析图')
        self.checkBox2 = QCheckBox('事件概率差排序图')
    ...
    def onCheckBoxChanged(self, state):
        ...
        if sender == self.checkBox1:
            if state == Qt.Checked:
                print("事件概率分析图已选中")
                self.figuretypes.append('事件概率分析图')
            else:
                print("事件概率分析图未选中")
                self.figuretypes.remove('事件概率分析图')
        elif sender == self.checkBox2:
            if state == Qt.Checked:
                print("事件概率差排序图已选中")
                self.figuretypes.append('事件概率差排序图')
            else:
                print("事件概率差排序图未选中")
                self.figuretypes.remove('事件概率差排序图')
