...
class Widget(OWWidget):
    ...
    name = "shap算法多特征分类可解释性分析"
    description = "shap算法多特征分类可解释性分析"
    ...
    #         self.data = runmain.data_read(user_input)
    #         self.read()
    #     else:
    #         self.data = None
    ...
    dataYLD_funcType_list: list = ['井名索引', '层号索引', '顶深索引', '底深索引', '深度索引', '目标', '特征',
                                   '其他目标',
                                   '忽略', 'x',
                                   'y', 'z']
    dataWDZ_type_list: list = ['常规数值', '指数数值', '文本', '其他']  # 微地震数据类型选择列表
    dataWDZ_funcType_list: list = ['井名索引', '层号索引', '顶深索引', '底深索引', '深度索引', '目标', '特征',
                                   '其他目标',
                                   '忽略', 'x',
                                   'y', 'z']
    ...
    def onComboBoxIndexChanged1(self, index):
        ...
        self.Y_name = selected_text1
        print(f"Y_name：{selected_text1}", self.Y_name)

    def onComboBoxIndexChanged121(self, index):
        # 获取当前选择的文本
        selected_text121 = self.comboBoxleft121.currentText()
        self.Target = selected_text121
        print(f"Target：{selected_text121}", self.Target)
    ...
    def onComboBoxIndexChanged4(self, index):
        # 获取当前选择的文本
        selected_text4 = self.comboBoxleft4.currentText()
        self.showtype = selected_text4
        print(f"当前选择othernames是：{selected_text4}", self.showtype)

    modelltype = '卡方分布模型'

    mdye = '特征选择数'

    def onComboBoxIndexChangedSF(self, index):
        # 获取当前选择的文本
        selected_text = self.comboBoxleftSF.currentText()
        self.modelltype = selected_text
        print(f"当前选择modeltype是：{selected_text}", self.modelltype)
    ...
    def onComboBoxIndexChanged(self, index):
        ...
        if selected_text == '特征选择数':
            ...
            self.label6.hide()
            self.input7.hide()
        print(f"当前选择modetype是：{selected_text}", self.mdye)
    ...
    def __init__(self):
        ...
        # 创建标签
        label1 = QLabel('Y_name:')
        label121 = QLabel('Target:')
        labelSF = QLabel('算法选择:')
        label2 = QLabel('决策模式:')
        label3 = QLabel('多选:')
        label4 = QLabel('num_feats:')
        ...
        self.comboBoxleft121 = QComboBox()
        self.comboBoxleft121.addItems(['请选择'])

        self.comboBoxleftSF = QComboBox()
        self.comboBoxleftSF.addItems(['请选择'])
        alist = ['xgboost分类算法', 'LGBMC分类算法',  'CatBoost分类算法',
                         '随机森林分类算法',  '决策树分类算法','梯度提升树分类算法', 'AdaBoost树分类算法', '外联树分类算法']

        self.comboBoxleftSF.addItems(alist)
        self.comboBoxleftSF.currentIndexChanged.connect(self.onComboBoxIndexChangedSF)
        ...
        # 创建下拉框
        self.comboBoxleft3 = QComboBox()
        self.comboBoxleft3.addItems(['综合特征分析', '各类特征分析'])
        self.comboBoxleft3.currentIndexChanged.connect(self.onComboBoxIndexChanged4)


        # 创建多选框['shap散点交会图', '分类别shap散点交会图', 'shap特征排序图', '特征组合影响图', '决策曲线图',
        # #             '平均shap值的条形图', '热力图']
        self.checkBox1 = QCheckBox('shap特征排序图')
        self.checkBox2 = QCheckBox('shap散点交会图')
        self.checkBox3 = QCheckBox('分类别shap散点交会图')
        # self.checkBox4 = QCheckBox('瀑布图')
        # self.checkBox5 = QCheckBox('交互依赖图')
        self.checkBox6 = QCheckBox('平均shap值的条形图')
        self.checkBox7 = QCheckBox('特征组合影响图')
        # self.checkBox8 = QCheckBox('条形图')
        self.checkBox9 = QCheckBox('决策曲线图')
        # self.checkBox10 = QCheckBox('蜂窝图')
        # self.checkBox11 = QCheckBox('平均shap值的条形图')
        ...
        # self.checkBox13.stateChanged.connect(self.onCheckBoxChanged)

        # 创建输入框

        self.input4 = QLineEdit()
        self.input5 = QLineEdit()
        self.input5.setPlaceholderText('请输入整数,默认为4')

        self.input6 = QLineEdit()
        self.input6.setPlaceholderText('请输入浮点数,默认为0.5')
        self.input7 = QLineEdit()
        self.input7.setPlaceholderText('请输入整数,默认为10')
        ...
        hbox121 = QHBoxLayout()
        hbox121.addWidget(label121)
        hbox121.addWidget(self.comboBoxleft121)

        hbox22 = QHBoxLayout()
        hbox22.addWidget(label2)
        ...
        hboxSF.addWidget(self.comboBoxleftSF)
        ...
        self.layoutBOTTOM.addLayout(hbox121)
        self.layoutBOTTOM.addLayout(hboxSF)
        self.layoutBOTTOM.addLayout(hbox22)
        self.layoutBOTTOM.addLayout(hbox2)
        self.layoutBOTTOM.addLayout(hbox3)
        self.layoutBOTTOM.addLayout(hbox33)
        self.layoutBOTTOM.addLayout(hbox333)
        self.layoutBOTTOM.addLayout(hbox3333)
        self.layoutBOTTOM.addLayout(hbox33333)
        self.layoutBOTTOM.addLayout(hbox333333)
        # self.layoutBOTTOM.addLayout(hbox3333333)
    ...
    def onCheckBoxChanged(self, state):
        sender = self.sender()
        figtypes = ['shap特征排序图', 'shap散点交会图', '分类别shap散点交会图', '瀑布图', '交互依赖图',
                    '平均shap值的条形图',
                    '特征组合影响图', '条形图', '决策曲线图', '蜂窝图', '平均shap值的条形图', '依赖图', '聚类特征图']
        if state == Qt.Checked:
            print(f"{sender.text()}已选中")
            self.figuretypes.append(sender.text())
        else:
            print(f"{sender.text()}未选中")
            self.figuretypes.remove(sender.text())
