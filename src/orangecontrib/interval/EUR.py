...
class Widget(OWWidget):
    ...
    # 这些列名(小写)将自动识别为时间索引
    time_col_alias = ['time', 'date', 'datetime', '日期', '时间', 'date_column']
    ...
    def ignore_function(self, text, prop):
        ...
        if text == '忽略':
            ...
            self.date_column = prop
            print("时间索引被选择，执行相应的函数", self.date_column)
        # 设置井名索引
        elif text == '井名索引':
            self.WellnameNEW = prop
            print("井名索引被选择，执行相应的函数", self.WellnameNEW)
    ...
    def run(self):
        ...
        # timeseriers_prediction(input_path, features, date_column='日期', wellname='井名',
        #                        wellnames=['GY1-Q1-H1', 'GY1-Q1-H2'], steps=200, predicttype='拟合及预测',
        #                        modetyps=['指数递减方程', '双曲线递减方程', '线性回归算法', 'GRU', 'LSTM'],
        #                        nanlsits=[-10000, -9999, -999.99, -999.25, -999, 999, 999.25, 9999, 0],
        #                        testsize=0.2, endday=3650, cutoff=0.1, foldername='时间序列EUR预测', outpath='输出路径',
        #                        savemode='.xlsx')

        # 打印所需参数
        print("文件路径：", self.user_inputpath)
        print("特征：", self.lognames)
        print("时间索引：", self.date_column)
        print("井名索引：", self.WellnameNEW)
        print('选中的井名', self.LEFTlist)
        print("预测类型：", self.predicttype)
        print("modetyps：", self.modetyps)

        # 调用主函数
        # result = runmain.timeseriers_prediction(self.user_inputpath, self.lognames, date_column=self.date_column,
        #                                         wellname=self.WellnameNEW, wellnames=self.LEFTlist, steps=self.steps,
        #                                         predicttype=self.predicttype, modetyps=self.modetyps,
        #                                         nanlsits=[-10000, -9999, -999.99, -999.25, -999, 999, 999.25, 9999, 0],
    ...
    def onComboBoxIndexChanged3(self, index):
        ...
        self.predicttype = selected_text3
        print(f"predicttype：{selected_text3}", self.predicttype)
    ...
    def fillPropTable(self, data: pd.DataFrame, tableName: str, table: QTableWidget, typeList: list,
        ...
        for i, prop in enumerate(properties):
            ...
            # 设置 时间索引
            elif prop.lower() in self.time_col_alias:
                self.propertyDict[tableName][prop]['funcType'] = funcTypeList[-1]
                self.date_column = prop
    ...
    def __init__(self):
        ...
        # 创建标签
        label7 = QLabel('predicttype:')
        label8 = QLabel(':')
        label9 = QLabel('截断值:')
        label10 = QLabel('steps:')
        ...
        lst = ['预测', '拟合', '拟合及预测']
        self.comboBoxleft3.addItems(lst)
        self.comboBoxleft3.currentIndexChanged.connect(self.onComboBoxIndexChanged3)
        self.comboBoxleft4 = QComboBox()

        # 创建输入框
        self.input9 = QLineEdit()
        self.input9.setPlaceholderText('默认为0.1')
        self.input10 = QLineEdit()
        self.input10.setPlaceholderText('请输入整数,默认为200')
        ...
        fiii = ['自回归', '移动平均模型', '自回归滑动平均模型', '差分整合移动平均自回归模型',
                '差分整合移动平均自回归模型', '季节性ARIMA']
        ...
        self.input6.setPlaceholderText('默认为0.2')
        self.input7 = QLineEdit()
        self.input7.setPlaceholderText('默认为3650')
        ...
        fillters = ['指数递减方程', '双曲线递减方程', '线性回归算法', 'GRU', '指数平滑', '线性趋势的方法']
        # self.checkboxes = []
