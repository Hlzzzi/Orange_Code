...
from .pkg import 二分类多特征排序选择算法 as runmain
...
class Widget(OWWidget):
    ...
    def set_datalist(self, datalist):
        if datalist:
            ...
            self.data = self._coerce_to_dataframe(datalist)
            self.read()
        else:
            self.ALLdata = None
            self.data = None
...
#         ##各项参数如下：
#         print('self.X_names:', self.lognames)
#         print('self.target_sj:', self.target_sj)
#
#
#         print('self.cut_corr:', self.cut_corr)
#         print('self.select_number:', self.select_number)
#         print('self.figuretypes:', self.figuretypes)
#         print('self.SI_type:', self.SI_type)
#         print('self.indextype:', self.indextype)
#
#
...
#
#         # (data, X_names, target, oternames=[], k=10, label11='正常', label22='套变',
...
#
#         resultPX , resultSX = runmain.features_choice_algorithm(self.data, self.lognames, self.target_sj, oternames=[], k=self.k,
#                                                         label11=self.label11, label22=self.label22, SI_type=self.SI_type, indextype=self.indextype,
#                                                           modeltype=self.modetype, select_number=self.select_number, cutoff=self.cut_corr, figtypes=self.figuretypes,
#                                                             )
#
#         self.save(resultPX)
#         self.save(resultSX)
...
#         self.Outputs.dataSX.send([resultSX])
    ...
    def run(self):
        ...
        if not hasattr(self, 'label11') or not self.label11 or self.label11 == '请选择':
            self.warning('请先选择目标1')
            return

        if not hasattr(self, 'label22') or not self.label22 or self.label22 == '请选择':
            self.warning('请先选择目标2')
            return
    ...
    def _run_rank_task(
        ...
        *,
        ...
        resultPX, resultSX = runmain.features_choice_algorithm(
            data,
            X_names,
            target,
            oternames=oternames,
    ...
    def _on_run_finished(self, future):
        try:
            ...
            print(e)
            self.error("二分类多特征排序选择算法运行失败，请检查目标列、特征列和参数设置")
            return
    ...
    def onComboBoxIndexChanged1(self, index):
        ...
        self.namedata12 = self.data[self.target_sj].dropna().astype(str).unique().tolist()
        ...
        self.comboBoxleft11.addItems(['请选择'] + self.namedata12)
        self.comboBoxleft22.addItems(['请选择'] + self.namedata12)
    ...
    def onComboBoxIndexChanged11(self, index):
        # 获取当前选择的文本
        selected_text11 = self.comboBoxleft11.currentText()
        self.label11 = selected_text11
        print(f"当前选择label11是：{selected_text11}", self.label11)

    def onComboBoxIndexChanged22(self, index):
        # 获取当前选择的文本
        selected_text22 = self.comboBoxleft22.currentText()
        self.label22 = selected_text22
        print(f"当前选择label22是：{selected_text22}", self.label22)
    ...
    def __init__(self):
        ...
        label8 = QLabel('K:')
        self.label5 = QLabel('阈阀值:')
        self.label6 = QLabel('特征选择数:')


        # 创建下拉框
        self.comboBoxleft1 = QComboBox()
        self.comboBoxleft1.addItems(['请选择'])

        self.comboBoxleft11 = QComboBox()
        self.comboBoxleft11.addItems(['请选择'])


        self.comboBoxleft22 = QComboBox()
        self.comboBoxleft22.addItems(['请选择'])

        self.comboBoxleft2 = QComboBox()
        self.comboBoxleft2.addItems(['特征选择数', '阈阀值特征选择'])
        self.comboBoxleft2.currentIndexChanged.connect(self.onComboBoxIndexChanged)

        label38 = QLabel('indextype:')
        self.comboBoxleft38 = QComboBox()
        self.comboBoxleft38.addItems(['请选择'])
        self.comboBoxleft38.addItems(['相似度系数', '相异度系数'])
        self.comboBoxleft38.currentIndexChanged.connect(self.onComboBoxIndexChanged9)

        label12 = QLabel('SI_type:')
        self.comboBoxleft12 = QComboBox()
        self.comboBoxleft12.addItems(['请选择'])
        self.comboBoxleft12.addItems(['GDOH1D', 'Jaccard1D'])
        self.comboBoxleft12.currentIndexChanged.connect(self.onComboBoxIndexChanged10)


        # 创建多选框['特征重叠度排序图', '特征归一化排序图', '特征贡献率排序图', '二分类优选特征频率直方图']
        self.checkBox1 = QCheckBox('特征重叠度排序图')
        self.checkBox2 = QCheckBox('特征归一化排序图')
        self.checkBox3 = QCheckBox('特征贡献率排序图')
        self.checkBox4 = QCheckBox('二分类优选特征频率直方图')
        ...
        self.input6.setPlaceholderText('请输入浮点数,默认为0.7')
        self.input7 = QLineEdit()
        self.input7.setPlaceholderText('请输入整数,默认为3')

        self.input8 = QLineEdit()
        self.input8.setPlaceholderText('请输入整数,默认为10')
        ...
        hbox11 = QHBoxLayout()
        hbox11.addWidget(label11)
        hbox11.addWidget(self.comboBoxleft11)
        hbox11.addWidget(label22)
        hbox11.addWidget(self.comboBoxleft22)
        ...
        hbox38 = QHBoxLayout()
        hbox38.addWidget(label38)
        hbox38.addWidget(self.comboBoxleft38)
        ...
        hbox33.addWidget(self.checkBox4)
        ...
        hboxk = QHBoxLayout()
        hboxk.addWidget(label8)
        hboxk.addWidget(self.input8)

        hboxSi = QHBoxLayout()
        hboxSi.addWidget(label12)
        hboxSi.addWidget(self.comboBoxleft12)

        self.hbox5 = QHBoxLayout()
        self.hbox5.addWidget(self.label5)
        self.hbox5.addWidget(self.input6)
        self.label5.hide()
        self.input6.hide()
        ...
        self.hbox6.addWidget(self.input7)

        self.hbox7 = QHBoxLayout()
        self.hbox7.addWidget(label8)
        self.hbox7.addWidget(self.input8)

        # 将布局添加到 QVBoxLayout
        self.layoutBOTTOM.addLayout(hbox1)
        self.layoutBOTTOM.addLayout(hbox11)
        self.layoutBOTTOM.addLayout(hbox2)
        self.layoutBOTTOM.addLayout(hbox38)
        self.layoutBOTTOM.addLayout(hbox3)
        self.layoutBOTTOM.addLayout(hbox33)
        # self.layoutBOTTOM.addLayout(hbox4)
        self.layoutBOTTOM.addLayout(hboxk)
        self.layoutBOTTOM.addLayout(hboxSi)
        self.layoutBOTTOM.addLayout(hbox7)
        self.layoutBOTTOM.addLayout(self.hbox5)
        self.layoutBOTTOM.addLayout(self.hbox6)

        containerrr9 = QWidget()
        # 设置容器的布局为 QVBoxLayout
        containerrr9.setLayout(self.layoutBOTTOM)
        # 将容器添加到 QGridLayout
        layout.addWidget(containerrr9, 0, 1)
    ...
    figuretypes = []
    indextype = None
    def onComboBoxIndexChanged9(self, index):
        self.indextype = self.comboBoxleft38.currentText()
        print(f"选择的变量: {self.indextype}")
    def onComboBoxIndexChanged10(self, index):
        self.SI_type = self.comboBoxleft12.currentText()
        print(f"选择的变量: {self.SI_type}")
